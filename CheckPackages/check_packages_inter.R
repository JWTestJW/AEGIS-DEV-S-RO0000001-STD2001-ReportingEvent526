...
ap <- available.packages(repos = internal_repo)
cat("取得完了。総パッケージ数: ", nrow(ap), "\n\n")

# ----------------------------------------------------------
# 存在チェック（内部リポジトリに存在しないパッケージを検出）
# ----------------------------------------------------------
not_found <- pkgs[!(pkgs %in% rownames(ap))]
if (length(not_found) > 0) {
  cat("[警告] 以下のパッケージは内部リポジトリに存在しません:\n")
...
output_file <- file.path(script_dir, "package_check_result_inter.csv")

csv_header <- paste0(
  '"パッケージ名",',
  '"バージョン（', today_str, '時点の内部リポジトリ最新）",',
  '"Depends",',
  '"Imports",',
  '"LinkingTo",',
  '"再帰依存（直接依存を除く）"'
...
dep_output_file <- file.path(script_dir, "dependency_info_inter.csv")
